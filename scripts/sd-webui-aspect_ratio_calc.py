            ...
            with InputAccordion(label="Aspect Ratio Calculator",
                ...
                with gr.Row(elem_id=layout2): 
                    arvalue = gr.Textbox(value="1:1", lines=1, render=True,
                        interactive=False, inputs=None, key="ARTextBox", label="",
                        info="Calculated aspect ratio from Width/Height"
                    )
                    prec = gr.Dropdown([0,1,2,3,4,5,6,7,8], label="", info="Precision", value="2")
                # Create a row.
                with gr.Row(elem_id=layout2):
                    ...
                    mybutton0 = gr.Button("Calculate AR", elem_id=layout3, min_width=50)
                    mybutton1 = gr.Button("Acquire W/H", elem_id=layout3, min_width=50) 
